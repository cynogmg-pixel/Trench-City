...
(function() {
    ...
    const THEME_STORAGE_KEY = 'tcTheme';
    ...
    /**
     * Read saved theme preference from storage
     */
    TC.getSavedTheme = function() {
        let theme = DEFAULT_THEME;
        try {
            const stored = window.localStorage ? window.localStorage.getItem(THEME_STORAGE_KEY) : null;
    ...
    /**
     * Apply theme to root + update dependent UI (logos, toggle state)
     */
    TC.applyTheme = function(theme) {
        ...
        logos.forEach((logo) => {
            ...
            const nextSrc = theme === 'light' ? (lightSrc || logo.getAttribute('src')) : (darkSrc || logo.getAttribute('src'));
     ...
     */
    TC.initTheme = function() {
        let currentTheme = TC.getSavedTheme();
        TC.applyTheme(currentTheme);

        const toggles = document.querySelectorAll('[data-theme-toggle]');
        console.log('[TC] Theme init - found', toggles.length, 'toggle button(s)');

        if (!toggles.length) {
            console.warn('[TC] No theme toggle buttons found');
            return;
        }

        toggles.forEach((btn, index) => {
            console.log('[TC] Attaching click listener to theme toggle', index);
            btn.addEventListener('click', () => {
                console.log('[TC] Theme toggle clicked!');
                currentTheme = currentTheme === 'light' ? 'dark' : 'light';
                console.log('[TC] Switching to theme:', currentTheme);
                try {
    ...
    TC.initSidebar = function() {
        ...
        console.log('[TC] Sidebar init - found collapse button:', !!collapseBtn);

        if (!sidebar) {
            console.warn('[TC] Sidebar element not found');
            return;
        }
        ...
        if (collapseBtn) {
            console.log('[TC] Attaching click listener to collapse button');
            collapseBtn.addEventListener('click', (e) => {
                console.log('[TC] Collapse button clicked!');
                const collapsed = body.classList.toggle('tc-sidebar-collapsed');
                console.log('[TC] Sidebar collapsed state:', collapsed);
                try {
            ...
            console.warn('[TC] Collapse button not found in DOM');
        }
    ...
    /**
     * Initialize dashboard widgets (collapse + tutorial toggle)
     */
    TC.initDashboardWidgets = function() {
        ...
        widgets.forEach((widget) => {
            ...
            if (!widgetId || !toggle) return;

            const storageKey = `tc_dash_widget_${widgetId}`;
        ...
        const tutorialBody = document.querySelector('.tc-tutorial-body');
        if (!tutorialToggle || !tutorialBody) return;

        const tutorialKey = 'tc_dash_tutorial_hidden';
        const applyTutorialState = (hidden) => {
            tutorialBody.classList.toggle('is-hidden', hidden);
            tutorialToggle.textContent = hidden ? 'Show' : 'Hide';
            tutorialToggle.setAttribute('aria-expanded', hidden ? 'false' : 'true');
        };

        let tutorialHidden = false;
        try {
            tutorialHidden = window.localStorage ? window.localStorage.getItem(tutorialKey) === '1' : false;
        } catch (err) {
            tutorialHidden = false;
        }
        applyTutorialState(tutorialHidden);

        tutorialToggle.addEventListener('click', () => {
            const isHidden = tutorialBody.classList.toggle('is-hidden');
            applyTutorialState(isHidden);
            try {
                if (window.localStorage) {
                    window.localStorage.setItem(tutorialKey, isHidden ? '1' : '0');
                }
            } catch (err) {
                /* ignore storage errors */
            }
