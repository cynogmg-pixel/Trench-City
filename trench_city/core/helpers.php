...
/**
 * Torn-like training softcap multiplier (monotonic, asymptotic floor).
 */
function tcSoftcapMultiplierForTraining(float $statValue): float
{
    $s = max(0.0, $statValue);
    $a = 1.4;
    $b = 1000.0;

    $m = 1.0 / (1.0 + ($a * log10(1.0 + ($s / $b))));
    $m = max(0.02, min(1.05, $m));

    if ($s < 1000.0) {
        $m *= (1.10 - (0.10 * ($s / 1000.0)));
    }
...
/**
 * Torn-like combat softcap multiplier (for marginal contribution).
 */
function tcSoftcapMultiplierForCombat(float $statValue): float
{
    $s = max(0.0, $statValue);
    $c = 0.85;
    $d = 5000.0;

    $m = 1.0 / (1.0 + ($c * log10(1.0 + ($s / $d))));
    $m = max(0.10, min(1.00, $m));
...
function tc_set_global_announcement(?array $data): bool
{
    ...
    if ($data === null) {
        ...
        $deleted = false;
        foreach ($paths as $candidate) {
            if (file_exists($candidate) && is_writable($candidate) && @unlink($candidate)) {
                $deleted = true;
            }
        }
        if ($deleted || $path === '' || !file_exists($path)) {
            return true;
        }

        $emptyPayload = json_encode(['message' => ''], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
        foreach ($paths as $candidate) {
            $dir = dirname($candidate);
            if (!is_dir($dir) || !is_writable($dir)) {
                continue;
            }
            if (@file_put_contents($candidate, $emptyPayload, LOCK_EX) !== false) {
                return true;
            }
        }
